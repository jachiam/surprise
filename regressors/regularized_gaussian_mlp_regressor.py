...
class GaussianMLPRegressor(LasagnePowered, Serializable):
    ...
    def __init__(
        ...
        weight_decay_term = 0.5 * sum([TT.sum(TT.square(param))
                                for param in self.get_params(regularizable=True)])

        if self._weight_decay > 0:
            loss = loss + self._weight_decay * weight_decay_term

        self._param_norm = compile_function([],weight_decay_term)
    ...
    def fit(self, xs, ys):
        ...
        logger.record_tabular(prefix + 'ParamL2',self._param_norm())
