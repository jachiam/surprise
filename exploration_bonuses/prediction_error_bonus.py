...
from sandbox.surprisebased.exploration_bonuses.base import ExplorationBonus
...
class PredictionErrorBonus(ExplorationBonus, Serializable):
    ...
    def __init__(
        ...
        super(PredictionErrorBonus, self).__init__()
        ...
        self.use_square_error = use_square_error
    ...
    def get_bonus(self, path):
        xs = np.concatenate( [ path["observations"][:-1], path["actions"][:-1] ], axis=1)
        ys = path["observations"][1:]
        if not(self.use_square_error):
            pred_err = np.sqrt(np.sum(np.square(ys - self._regressor.predict(xs)),axis=1))
        else:
            pred_err = np.sum(np.square(ys - self._regressor.predict(xs)),axis=1)
        pred_err = np.append(pred_err,[0])
        return pred_err
